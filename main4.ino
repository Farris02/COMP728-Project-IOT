...
const int grace_period = 30 * 60 * 1000; //grace period (30 mins)
const int week = 7 * 24 * 60 * 60 * 1000; // week (7 days)

//int infraredValue; // 0 is not closed, 1 is closed.
DHT dht = DHT(humitureSensor, DHT11);

unsigned long lastWindowChangeTimes[7] = {0}; // array to store last 7 days
unsigned int Index_of_Last_Window_Change = 0; // sets index of next window change time
...
void loop() {
  ...
  bool hasSentHumidityMessage = false;
  bool hasSentTemperatureMessage = false;
  bool consistent_window_change = false;
...
//This checks if the window has been opened/closed on a consistent basis over the last 7 days.
for(int i = 0; i < 7; i++)
{
if(lastWindowChangeTimes[i] != 0 && lastWindowChangeTimes[i] + week >= currentTime && 
lastWindowChangeTimes[i] + week - grace_period <= currentTime) {
  consistentWindowChange = true;
  break;

}
}

//Below sets the window status based on the consistent pattern of window changes of the last 
if(consistentWindowChange){
supposedWindowClosedStatus= !currentWindowClosed
}
else {
    supposedWindowClosedStatus = wasRaining || hasSentHumidityMessage || hasSentTemperatureMessage;
  }


//This code allows to keep track of the last 7 days of manual window changes and to adjust the window status accordingly based on consistent patterns of manual changes.
if(supposedWindowClosedStatus != currentWindowClosedStatus)
...
else if(startWaitWindowChangeTime != 0 && startWaitWindowChangeTime + grace_period <= currentTime){
  startWindowChangeTime = 0;
  for(int i = 0; i<7; i++){
    if(lastWindowChangeTimes[i] == && startWaitWindowChangeTime){
      lastWindowChangeTimes[i] = currentTime;
      break;
    }
